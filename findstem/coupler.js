...
const by_start = function (p, q) { return p[0].x - q[0].x; };
...
function by_yori(a, b) { return b[0].y - a[0].y; }
function by_xori(a, b) { return b[0].y - a[0].y; }
...
function pairSegments(radicals, strategy) {
	...
	return stems.sort(function (a, b) { return a.y - b.y; });
}
...
function pairSymmetricStems(stems, strategy) {
	...
	for (var j = 0; j < stems.length; j++) {
		for (var k = j + 1; k < stems.length; k++) if (stems[j] && stems[k]) {
			...
			if (
				Math.abs(stems[j].y - stems[j].width / 2 - stems[k].y + stems[k].width / 2) <= strategy.UPM * delta1 && Math.abs(stems[j].width - stems[k].width) <= strategy.UPM * delta1
			) {
